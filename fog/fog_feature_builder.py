...
def _estimate_fs(timestamps: np.ndarray) -> float:
    ...
    diffs = diffs[diffs > 0]
    return 30.0 if len(diffs) == 0 else 1000.0 / np.mean(diffs)
...
def _features_b(window: dict, fs: float) -> dict:
    ...
    feats.update(_jerk_halves(_jerk(acc_ankle, ts), "Acc_Ankle"))
    return feats
...
def _features_c(window: dict, fs: float) -> dict:
    ...
    feats.update(_jerk_halves(_jerk(acc_ankle, ts),           "Acc_Ankle"))
    feats.update(_jerk_halves(_jerk(gyro_lsh, lshank["timestamp"]), "Gyro_LShank"))
    return feats
